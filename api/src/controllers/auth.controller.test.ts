...
describe('AuthController', () => {
  ...
  describe('self', () => {
    ...
    it('should check if a users auth token matches one found in the database', async () => {
      ...
      const authController = new AuthController();
      const response = await authController.self(
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6IjEyM0BnbWFpbC5jb20iLCJpYXQiOjE1MTYyMzkwMjJ9.I5XbzYKaGyeKykmcDu-tuWBE4E68lfuQTBJYVKwMb2c'
      );
    ...
    it('should throw and error if the provided token is invalid', async () => {
      ...
      try {
        await authController.self(
          'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6IiIsImlhdCI6MTUxNjIzOTAyMn0.RJplmJsNNH_flBFVwDIBVnjnZD6mpuH8CRErcG6R3KQ'
        );
      } catch (e) {
        err = e;
      }
