...
export class ClassroomController
{
    ...
    async update(classroom: IClassroom): Promise<void>
    {
        try{
            ...
            const classroom_current = await ClassroomModel.findOne({BuildingName: classroom.BuildingName, BuildingId: classroom.BuildingId, RoomNumber: classroom.RoomNumber}).catch((err) => err);
            if(!classroom_current) //insert if the classroom doesn't exist
            {
                await classroom_db.save(classroom_db).catch((err) => err);
            }else //replace if exists
            {
                const doc = await ClassroomModel.findOne({BuildingName: classroom.BuildingName, BuildingId: classroom.BuildingId, RoomNumber: classroom.RoomNumber}).catch((err) => err);
